...
3. **完成数学任务。** 选择自然入口，展开决定结论的步骤，只检查本题实际使用的条件。一般教学行为按 `references/learning-protocol.md`；显式 K 编号深度讲义以专用 reference 为准。
...
- 只聊天的直接解答不读取 catalog、registry、模板或 references；但输入含 `Kxxx` 且答案依赖编号含义时，允许且必须只运行一次提取器解析编号，不因这一步触发深度模式或持久化。只有用户询问仓库现有资产时才查其他事实源。
...
- `references/deep-lecture-by-id.md`：用户显式要求“深度讲解 / 生成深度讲义 / 满分级讲义 + Kxxx”时；普通“讲解 Kxxx”只运行提取器解析含义，不得加载本 reference 或机械套用。
