...
\begin{itemize}
  ...
  \item \textbf{成立条件：} “任意点列”是关键；构造一个反例点列可否定极限。
  \item \textbf{典型用法与识别信号：} 证明极限不存在；构造反例；识别信号为 “沿不同路径/序列”。
...
\begin{knowledgeBox}
\textbf{定义或结论：} 若 $\alpha(x)\to0$，称其为无穷小；若 $|f(x)|\to\infty$，称为无穷大。非零无穷小的倒数为无穷大，反之亦然。

\begin{itemize}
  \item \textbf{直觉与必会动作：} 会在指定过程下判断；区分“无穷小量”与常数0。
  \item \textbf{成立条件：} 必须明确趋近过程；倒数结论要求在去心邻域非零。
  \item \textbf{典型用法与识别信号：} 判断无穷小阶；极限改写；识别信号为 “无穷小量”“无穷大量”。
  \item \textbf{关系与迁移：} 前置知识为 极限定义；可迁移到 等价替换、级数必要条件、反常积分。
  \item \textbf{常见误区：} 把很小的固定数称无穷小；忽略过程。迁移到新题时先复核定义域、趋近方向和所用定理的条件。
\end{itemize}
\end{knowledgeBox}
