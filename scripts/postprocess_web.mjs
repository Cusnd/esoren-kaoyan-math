...
const SITE_DIR = path.resolve(process.argv[3] ?? path.join(ROOT, 'build', 'site'));
const PDF_PATH = path.resolve(process.argv[4] ?? path.join(ROOT, 'build', 'pdf', 'main.pdf'));
const SCHEMA_VERSION = 1;
const SITE_ORIGIN = String(process.env.SITE_ORIGIN ?? 'https://kaoyan-math1-notes.sorenliu.workers.dev').replace(/\/$/, '');
  ...
  return groupPages(pages).map((group, subjectIndex) => {
    ...
    const lectures = groupByLecture(group.pages).map((lecture) => {
      ...
      if (lecture.pages.length === 1) {
        ...
        return `<li><a class="reader-toc__link reader-toc__lecture-link" href="/${page.slug}"${page.slug === currentSlug ? ' aria-current="page"' : ''}>${escapeHtml(page.title)}</a></li>`;
      }
      ...
      const childLinks = lecture.pages.map((page, pageIndex) => {
        ...
        return `<li><a class="reader-toc__link" href="/${page.slug}"${page.slug === currentSlug ? ' aria-current="page"' : ''}><span class="reader-toc__index" aria-hidden="true">${index}</span><span>${escapeHtml(shortTitle)}</span></a></li>`;
      }).join('');
...
function dialogsMarkup() {
  ...
  <dialog id="reader-preferences" aria-labelledby="reader-preferences-title">
    ...
    <div class="reader-dialog__body">
      ...
      <a class="reader-download-link" href="/downloads/kaoyan-math1-notes.pdf" download>${icon('download')} 下载完整 PDF</a>
...
function topbarMarkup() {
  return `<header class="reader-topbar">
    ...
    <a class="reader-brand" href="/">考研数学一—满分学习笔记</a>
    ...
    <nav class="reader-toolbar" aria-label="阅读工具">
      ...
      <a href="/downloads/kaoyan-math1-notes.pdf" target="_blank" rel="noopener" aria-label="在新窗口打开 PDF"><span class="reader-action__label">PDF</span></a>
    </nav>
  </header>`;
}
...
function pageNavMarkup(previous, next) {
  const item = (page, direction) => page ? `<a rel="${direction}" href="/${page.slug}">${icon(direction === 'prev' ? 'arrowLeft' : 'arrowRight')}<span><small>${direction === 'prev' ? '上一篇' : '下一篇'}</small>${escapeHtml(page.title)}</span></a>` : '<span></span>';
  return `<nav class="reader-page-nav" aria-label="前后篇">${item(previous, 'prev')}<a href="/">全部讲次</a>${item(next, 'next')}</nav>`;
}
...
function homeMarkup(pages) {
  ...
  return `<div class="reader-home">
    ...
    <a class="reader-continue" href="/${pages[0].slug}" data-reader-continue hidden>继续阅读：<span data-reader-continue-title>${escapeHtml(pages[0].title)}</span></a>
    <nav class="reader-index-links" aria-label="复习索引">${pages.filter((page) => page.subject === '复习索引').map((page) => `<a href="/${page.slug}">${escapeHtml(page.title)}</a>`).join('')}</nav>
    ${groups.filter((group) => group.subject !== '复习索引').map((group) => `<section class="reader-home__subject"><h2>${escapeHtml(group.subject)}</h2><div class="reader-home__lectures">${groupByLecture(group.pages).map((lecture) => `<section class="reader-home__lecture"><h3>${escapeHtml(lecture.lecture)}</h3><ol>${lecture.pages.map((page) => `<li><a href="/${page.slug}">${escapeHtml(page.title)}</a></li>`).join('')}</ol></section>`).join('')}</div></section>`).join('')}
  </div>`;
}
...
function canonicalLink(href, byLegacy, bySlug, byLwarpSlug) {
  ...
  if (/^(?:\.\/)?index(?:\.html)?$/.test(target)) return `/${hash ? `#${hash}` : ''}`;
  const note = target.match(/(?:^|\/)note-(\d+)(?:\.html)?$/);
  if (note && byLegacy.has(Number(note[1]))) return `/${byLegacy.get(Number(note[1])).slug}${hash ? `#${hash}` : ''}`;
  const basename = target.replace(/^\.\//, '').replace(/\.html$/, '');
  if (bySlug.has(basename)) return `/${basename}${hash ? `#${hash}` : ''}`;
  if (byLwarpSlug.has(basename)) return `/${byLwarpSlug.get(basename).slug}${hash ? `#${hash}` : ''}`;
  return href;
}
...
async function writeDeploymentFiles(pages, buildId) {
  ...
  await cp(path.join(pwaRoot, 'manifest.webmanifest'), path.join(SITE_DIR, 'manifest.webmanifest'));
  const offline = (await readFile(path.join(pwaRoot, 'offline.html'), 'utf8'))
    .replaceAll('__ASSET_ROOT__', `/assets/${buildId}`);
  ...
  const redirectLines = pages.flatMap((page) => [
    `/note-${page.legacyNote} /${page.slug} 301`,
    `/note-${page.legacyNote}.html /${page.slug} 301`,
  ]);
  if (redirectLines.length !== 104) fail(`Expected 104 legacy redirects, found ${redirectLines.length}.`);
  await writeFile(path.join(SITE_DIR, '_redirects'), `${redirectLines.join('\n')}\n`, 'utf8');

  const htmlHeaders = ['/', ...pages.map((page) => `/${page.slug}`), '/offline.html', '/manifest.webmanifest', '/data/*']
...
/assets/icons/v1/*
  Cache-Control: public, max-age=31536000, immutable
`;
  await writeFile(path.join(SITE_DIR, '_headers'), headers, 'utf8');
  ...
  const precache = [
    ...
    ...assetFiles.map((file) => `/${path.relative(SITE_DIR, file).replaceAll('\\', '/')}`),
  ];
  const physicalFiles = [
  ...
  const legacyRoutes = Object.fromEntries(pages.flatMap((page) => [
    [`/note-${page.legacyNote}`, `/${page.slug}`],
    [`/note-${page.legacyNote}.html`, `/${page.slug}`],
  ]));
...
async function main() {
  ...
  const buildSeed = [manifestSource, ...rawPages.flatMap(({ page, source }) => [page.slug, source])];
  const seedFiles = [
  ...
  const publicPages = pages.map((page, index) => ({ ...page, url: `/${page.slug}`, previous: pages[index - 1]?.slug ?? null, next: pages[index + 1]?.slug ?? null }));
  const assetRoot = `/assets/${buildId}`;
  await writeFile(path.join(SITE_DIR, 'data', 'content-manifest.json'), JSON.stringify({ schemaVersion: SCHEMA_VERSION, buildId, home: '/', assets: { root: assetRoot, reader: `${assetRoot}/reader`, mathjax: `${assetRoot}/vendor/mathjax-3.2.2` }, pages: publicPages, pdf: { url: '/downloads/kaoyan-math1-notes.pdf', sha256: pdfSha256, bytes: pdf.length } }, null, 2), 'utf8');
