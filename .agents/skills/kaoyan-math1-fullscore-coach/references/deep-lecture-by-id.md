...
先按一般学习协议在内部建立“目标结论—直接前置—使用位置”的依赖链，再组织正文；提取结果中的“前置知识”是重要线索但不是穷尽清单，还要按实际数学推导补齐必要依赖。内部依赖链不必作为固定表格公开。

根据知识点的概念、公式、定理、方法或大纲边界属性自然组织，不机械输出固定标题。正文从最小具体情形进入，依次接好必要前置、目标定义与推导、方法流程、梯度例题和配套训练；数学上适用的下列内容必须全部闭合：

- **定位与路线：** 解决什么问题、后续用途和本讲顺序；直接前置不能只列名，必须在首次调用前完成一般学习协议规定的四项闭环。
...
在生成例题、练习或发布正文前，逐项检查所有新出现且承担推导作用的术语、符号、定理和方法：

1. 直接前置必须已经定义或回顾，并说明它在目标结论中的具体作用，配有最小例子或反例；
2. 持久化或独立讲义中，已略过的内容必须在目标正文中已有掌握证据；只有互动聊天可使用当前对话证据，不能因读者没有发问就假定其会；
3. 高阶观点若不展开，必须明确标为可选，并保证删去它后主定义、主证明和考场方法仍然完整；
4. 发现任何悬空依赖时先补讲或改写主论证，未闭合前不得进入 A/B/C/M 练习或发布。

例如讲闭区间最值时，若正文使用“紧集、上确界、Fermat 必要条件、驻点、局部极值”等概念，就必须在使用前讲明；若“紧集、上确界”只是高阶视角，则标成可选旁注，寻找候选点和证明候选完备性的主线不能只依赖这些未展开概念。
