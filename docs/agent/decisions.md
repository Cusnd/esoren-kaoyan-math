...
## 2026-07-18 GPT-5.6 学习系统边界

- 提示层继续采用一个短小稳定的根 `AGENTS.md` 和一个仓库数学 Skill；教学、归档与仓库编辑细节分别按需加载，其他 Code Agent 只使用薄启动提示，不安装或复制平行规则。
- 题目继续使用一个 `data/problem_registry.yml` 和一个共享题号空间，以 `collection: core | practice` 区分主库与练习库；练习晋升时只改变库别，不更换稳定 ID。
- 主库只保留母题、新方法、关键知识载体与真实错题；普通变式和各阶段训练进入练习库。主 PDF 不混入练习，练习册与答案册独立生成，Web 统一展示两库。
- 知识关系只由稳定知识节点和显式边表达；通用陷阱与用户真实错题分离。复习只调度知识节点，题目只作为复习证据。
- 阅读器保持纯静态、本地优先，不引入账号、鉴权、后端、网页模型调用或答题评分；个人复习状态只保存在浏览器，并提供版本化导入导出。
- 提示词变化采用可执行前向评测逐组验证；模型与推理档位属于评测参数，不写死在仓库提示词中。任何可能写文件的模型评测只在隔离快照执行。
- Windows 前向评测默认在 WSL `workspace-write` 外叠加 bubblewrap：只绑定一次性 workspace 与仓库外空 output，以空 tmpfs 逐个遮蔽 `/mnt/[a-z]` Windows 盘符并保留 DNS 所需 `/mnt/wsl`，在结果中记录隔离协议；基础仓库在运行开始时只冻结一次，提示词 A/B 只覆盖 `AGENTS.md`、Skill 与 references。人工复核使用分离答案键的盲审包，每个 run 按顶层完整 fatal taxonomy 复核。
- 每个前向案例在应用 fixture 后初始化一次性本地 Git 基线，使模型能按项目契约检查真实工作树；`.git` 不进入内容 diff。等价数学写法与教学措辞由语义候选匹配，授权遵守由实际 diff 判断，不用冗余固定话术代替结果证据。
- 中途可恢复的工具错误保留在 `automatic_pass` 与资源效率证据中，但不单独升级为计划未列出的致命失败；越权路径、错误答案、缺失归档、验证不实、超时或其他结果门禁仍会阻止候选通过。每次调用默认最多 900 秒，超时原样计入，不重跑或删样本。
- 输出正则只作自动启发式与盲审定位，不机械宣布数学语义致命失败；语义 fatal 必须由盲审复核。automatic-fatal 政策变化使用保留来源摘要哈希的重分类产物，不覆盖原始 summary，也不重跑或改写模型输出。

## 2026-07-21 高数研究包与知识 registry v2

- `resources/manifest.yml` 只登记研究包的文件角色、哈希和投影边界；三份高数资料是同一数据集的不同视图，不取代教材 catalog、题目 registry 或数学正文，也不成为构建输入。
- `data/knowledge_registry.yml` 升级为 schema v2：`source_refs` 只用于来源审计，外部 K/T 编号通过别名进入搜索；`reviewable` 默认 `true`，明确的边界知识设为 `false`。
- 研究资料中的考频、趋势、研究评级、学习状态、复盘次数、日期和个人笔记不进入公开 registry、PDF 或 Web；无法从公开官方页面逐条确认的范围判断不写成官方结论。
- 全部知识节点仍进入静态搜索与关系索引；边界知识显示为“边界知识”，但复习界面不提供加入操作，状态导入遇到边界节点时整包拒绝并保持本地状态不变。
- 本轮只扩展高数 18 讲映射；`calc-07` 保留为显式覆盖缺口，不借用其他讲次条目填充。线性代数、概率论与现有题号、旧页面和旧锚点保持不变。
